...
export const update = async (id: string, data: EventsUpdateData) => {
    try {
...
export const doMatches = async (id_event: string): Promise<boolean> => {
    const eventItem = await prisma.event.findFirst({ where: { id: id_event }});
    
    if (eventItem) {
        const peopleList = await people.getAll({ id_event });
        ...
        if (peopleList) {
            ...
            let attempts = 0;
            let maxAttempts = peopleList.length < 10 ? 10 : peopleList.length;
            let keepTrying = true;
